...
class DicewarGame(Hexagon, Territory, Player):
    def __init__(self, ):
        ...
        # Define hexagon during initialization
        self.hexagon_size = 20
        self.player_num = 8
        self.rows=15
        self.cols=20

        # Set up parameters in game
        self.max_dice = 8
        self.max_undistributed = 50
        self.button_rect = pygame.Rect(600, 30, 100, 40)
        self.human = -0

        self.generate_map(self.hexagons, self.territories, self.players)
        self.find_neighbor_territories(self.territories)
        for territory in self.territories:
            self.find_next_vertex(self.hexagons, territory, territory.hexagon[0].border_direction[0] , territory.hexagon[0])

    # Generate hexagon vertices
    def generate_hexagon_vertices(self, center_x, center_y):
        vertices = []
        for i in range(6):
            angle_deg = 60 * i
            angle_rad = math.radians(angle_deg)
            x = center_x + self.hexagon_size * math.cos(angle_rad)
            y = center_y + self.hexagon_size * math.sin(angle_rad)
            vertices.append((x, y))
        return vertices

    # Generate map territories
    def generate_map(self, hexagons, territories, players):
        
        # Calculate the total width and height of the hexagonal grid
        total_width = self.cols * 3/2 * self.hexagon_size
        total_height = (self.rows + 0.5) * 2 * self.hexagon_size * math.sin(math.radians(60))

        # Calculate the offset to center the grid on the screen
        offset_x = (self.screen_width - total_width) / 2
        offset_y = (self.screen_height - total_height) / 2
        ...
        for row in range(self.rows):
            for col in range(self.cols):
                center_x = offset_x + col * 3/2 * self.hexagon_size
                center_y = offset_y + row * (2 * self.hexagon_size * math.sin(math.radians(60)))
                if col % 2 == 1:
                    center_y += self.hexagon_size * math.sin(math.radians(60))
    ...
    def find_neighbor_territories(self, territories):
    ...
    def find_next_vertex(self, hexagons, current_territory, current_direction, current_hexagon):

        vertex = self.generate_hexagon_vertices(current_hexagon.center[0],current_hexagon.center[1])[(current_direction + 1) % 6]
        ...
        if current_direction == 0:
            if 1 in current_hexagon.border_direction:
                self.find_next_vertex(hexagons, current_territory, 1, current_hexagon)
            else:
                if 5 in hexagons[current_hexagon.index + self.cols].border_direction: 
                    self.find_next_vertex(hexagons, current_territory, 5, hexagons[current_hexagon.index + self.cols])
                else: print("ERROR")
        if current_direction == 1:
            if 2 in current_hexagon.border_direction:
                self.find_next_vertex(hexagons, current_territory, 2, current_hexagon)
            else:
                if 0 in hexagons[current_hexagon.index -1].border_direction and current_hexagon.col_index % 2 == 0: 
                    self.find_next_vertex(hexagons, current_territory, 0, hexagons[current_hexagon.index -1])
                elif 0 in hexagons[current_hexagon.index + self.cols - 1].border_direction and current_hexagon.col_index % 2 == 1: 
                    self.find_next_vertex(hexagons, current_territory, 0, hexagons[current_hexagon.index + self.cols -1])
                else: print("ERROR")
        if current_direction == 2:
            if 3 in current_hexagon.border_direction:
                self.find_next_vertex(hexagons, current_territory, 3, current_hexagon)
            else:
                if 1 in hexagons[current_hexagon.index -1].border_direction and current_hexagon.col_index % 2 == 1: 
                    self.find_next_vertex(hexagons, current_territory, 1, hexagons[current_hexagon.index -1])
                elif 1 in hexagons[current_hexagon.index - self.cols -1].border_direction and current_hexagon.col_index % 2 == 0: 
                    self.find_next_vertex(hexagons, current_territory, 1, hexagons[current_hexagon.index - self.cols -1])
                else: print("ERROR")
        if current_direction == 3:
            if 4 in current_hexagon.border_direction:
                self.find_next_vertex(hexagons, current_territory, 4, current_hexagon)
            else:
                if 2 in hexagons[current_hexagon.index - self.cols].border_direction: 
                    self.find_next_vertex(hexagons, current_territory, 2, hexagons[current_hexagon.index - self.cols])
                else: print("ERROR")
        if current_direction == 4:
            if 5 in current_hexagon.border_direction:
                self.find_next_vertex(hexagons, current_territory, 5, current_hexagon)
            else:
                if 3 in hexagons[current_hexagon.index +1].border_direction and current_hexagon.col_index % 2 == 1: 
                    self.find_next_vertex(hexagons, current_territory, 3, hexagons[current_hexagon.index +1])
                elif 3 in hexagons[current_hexagon.index - self.cols +1].border_direction and current_hexagon.col_index % 2 == 0: 
                    self.find_next_vertex(hexagons, current_territory, 3, hexagons[current_hexagon.index - self.cols +1])
                else: print("ERROR")
        if current_direction == 5:
            if 0 in current_hexagon.border_direction:
                self.find_next_vertex(hexagons, current_territory, 0, current_hexagon)
            else:
                if 4 in hexagons[current_hexagon.index +1].border_direction and current_hexagon.col_index % 2 == 0: 
                    self.find_next_vertex(hexagons, current_territory, 4, hexagons[current_hexagon.index +1])
                elif 4 in hexagons[current_hexagon.index + self.cols +1].border_direction and current_hexagon.col_index % 2 == 1: 
                    self.find_next_vertex(hexagons, current_territory, 4, hexagons[current_hexagon.index + self.cols +1])
                else: print("ERROR")

# A single turn for a single player 
def turn(current_player, players, territories, human = -1, button_rect = pygame.Rect(600, 30, 100, 40)):
    ...
    while running:
        ...
        if selected_attacker and selected_defender:
            display_map(selected_attacker, selected_defender, current_player, players, territories, button_rect)
...
def distribute_dice(current_player, territories, max_dice = 8, max_undistributed = 50):
    list = []
    for territory in territories:
...
if __name__ == "__main__":
    ...
    calculate_player_territory( game.players, game.territories )
    ...
    while game_continues:
        ...
        for i in game.players:
            if i.territories_num == 0:
                ...
                continue
            targets.append(turn(i, game.players, game.territories, human))
            if targets[-1] == "finish":
